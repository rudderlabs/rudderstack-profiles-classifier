...
class Processor(ABC):
    ...
    def _prepare_feature_table(
        ...
        try:
            ...
            if self.trainer.eligible_users:
                feature_table = self.connector.get_table(
                    self.session,
                    feature_table_name,
                    filter_condition=self.trainer.eligible_users,
                )
            else:
                default_user_shortlisting = (
                    f"{self.trainer.label_column} != {self.trainer.label_value}"
                )
                feature_table = self.connector.get_table(
                    self.session,
                    feature_table_name,
                    filter_condition=default_user_shortlisting,
                )  # .withColumn(label_ts_col, F.dateadd("day", F.lit(prediction_horizon_days), F.col(index_timestamp)))
            arraytype_features = self.connector.get_arraytype_features(
                self.session, feature_table_name
            )
            ignore_features = utils.merge_lists_to_unique(
                self.trainer.prep.ignore_features, arraytype_features
            )
            high_cardinal_features = self.connector.get_high_cardinal_features(
                feature_table,
            ...
            feature_table = self.connector.drop_cols(
                feature_table, [self.trainer.label_column]
            )
            timestamp_columns = self.trainer.prep.timestamp_columns
            if len(timestamp_columns) == 0:
                timestamp_columns = self.connector.get_timestamp_columns(
                    self.session, feature_table_name, self.trainer.index_timestamp
                )
            for col in timestamp_columns:
                feature_table = self.connector.add_days_diff(
                    feature_table, col, col, self.trainer.index_timestamp
                )
            label_table = self.trainer.prepare_label_table(
                self.connector, self.session, label_table_name, label_ts_col
            )
            ...
            feature_table = self.connector.join_feature_table_label_table(
                feature_table, label_table, self.trainer.entity_column, "inner"
            )
    ...
    def train(
        self, train_procedure, material_names: List[Tuple[str]], merged_config: dict
    ):
        ...
        for row in material_names:
            ...
            (
                ...
                arraytype_features,
                timestamp_columns,
            ) = self._prepare_feature_table(
                feature_table_name, label_table_name, cardinal_feature_threshold
            )
        ...
        filtered_feature_table = self.connector.filter_feature_table(
            feature_table,
            self.trainer.entity_column,
            self.trainer.index_timestamp,
            self.trainer.max_row_count,
        ...
        try:
            train_results_json = self._call_procedure(
                train_procedure,
                feature_table_name_remote,
                merged_config,
