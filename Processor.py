...
class Processor(ABC):
    ...
    def _prepare_feature_table(self, feature_table_name: str, 
        ...
        try:
            ...
            if self.trainer.eligible_users:
                feature_table = self.connector.get_table(self.session, feature_table_name, filter_condition=self.trainer.eligible_users)
            else:
                default_user_shortlisting = f"{self.trainer.label_column} != {self.trainer.label_value}"
                feature_table = self.connector.get_table(self.session, feature_table_name, filter_condition=default_user_shortlisting) #.withColumn(label_ts_col, F.dateadd("day", F.lit(prediction_horizon_days), F.col(index_timestamp)))
    ...
    def train(self, train_procedure, material_names: List[Tuple[str]], merged_config: dict):        
        ...
        for row in material_names:
            ...
            feature_table_instance, arraytype_features, timestamp_columns = self._prepare_feature_table(feature_table_name, 
                                                                    label_table_name,
        ...
        try:
            train_results_json = self._call_procedure(train_procedure,
