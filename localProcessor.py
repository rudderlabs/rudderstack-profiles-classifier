    ...
    def prepare_feature_table(self, session,
        ...
        try:
            ...
            if self.trainer.eligible_users:
                feature_table = self.connector.get_table_as_dataframe(session, feature_table_name, filter_condition=self.trainer.eligible_users)
            else:
                default_user_shortlisting = f"{self.trainer.label_column} != {self.trainer.label_value}"
                feature_table = self.connector.get_table_as_dataframe(session, feature_table_name, filter_condition=default_user_shortlisting) #.withColumn(label_ts_col, F.dateadd("day", F.lit(prediction_horizon_days), F.col(index_timestamp)))
