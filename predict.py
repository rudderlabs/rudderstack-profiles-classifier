...
def predict(creds:dict, aws_config: dict, model_path: str, inputs: str, output_tablename : str, config: dict) -> None:
    ...
    preds = preds.withColumn(output_label_column, utils.F.when(utils.F.col(score_column_name)>=prob_th, utils.F.lit(label_value)).otherwise(utils.F.lit(1-label_value)))
