...
class DatabaseManager:
    ...
    def upload_sample_data(self, sample_data_dir: str, table_suffix: str) -> dict:
        ...
        for filename in os.listdir(sample_data_dir):
            ...
            if table_name.lower() in existing_tables:
                ...
                action = self.input_handler.get_user_input("Do you want to skip uploading again, so we can reuse the tables? (yes/no) (yes - skips upload, no - uploads again): ")
                if action == "yes":
    ...
    def get_sample_data(self, table: str, column: str, num_samples: int = 5) -> List[str]:
        try:
            ...
            logger.error(f"Error fetching sample data for column '{column}' in table '{table}': {e}")
            return []

    def map_columns_to_id_types(self, table: str, id_types: List[str], entity_name: str) -> Tuple[List[Dict], str]:
        try:
        ...
        logger.info(f"\n{'-'*80}\n")
        logger.info(f"The table `{table}` has the following columns, which look like id types:\n")
