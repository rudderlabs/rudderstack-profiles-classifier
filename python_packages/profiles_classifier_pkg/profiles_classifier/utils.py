...
class utils:
        ...
        for key in profiles_config:
            if key in notebook_config:
                if isinstance(profiles_config[key], dict) and isinstance(notebook_config[key], dict):
                    merged_config[key] = utils.combine_config(notebook_config[key], profiles_config[key])
    ...
    def get_output_directory(folder_path: str, materialized_folder: str)-> str:
        ...
        Args:
            ...
            materialized_folder (str): Name of the materialized folder for current model

        Returns:
            str: output directory path
        """
        if folder_path is None:
            materialized_folder = "train"
    ...
    def get_latest_material_hash(session: snowflake.snowpark.Session,
        ...
        """
        snowpark_df = utils.get_material_registry_table(session, material_table)
    ...
    def get_material_names(session: snowflake.snowpark.Session, material_table: str, start_date: str, end_date: str, 
        ...
        """
        try:
            material_names, training_dates = utils.get_material_names_(session, material_table, start_date, end_date, features_profiles_model, model_hash, material_table_prefix, prediction_horizon_days)
    ...
    def get_best_th(y_true: pd.DataFrame, y_pred_proba: np.array,train_config: dict) -> Tuple:
        ...
        best_metrics = utils.get_classification_metrics(y_true, y_pred_proba, best_th)
        return best_metrics, best_th
    ...
    def get_metrics(clf,
        ...
        """
        train_preds = clf.predict_proba(X_train)[:,1]
        train_metrics, prob_threshold = utils.get_best_th(y_train, train_preds,train_config)

        test_preds = clf.predict_proba(X_test)[:,1]
        test_metrics = utils.get_classification_metrics(y_test, test_preds, prob_threshold)

        val_preds = clf.predict_proba(X_val)[:,1]
        val_metrics = utils.get_classification_metrics(y_val, val_preds, prob_threshold)
    ...
    def plot_top_k_feature_importance(session, pipe, stage_name, train_x, numeric_columns, categorical_columns, chart_name, top_k_features=5)-> None:
        ...
        try:
            ...
            onehot_encoder_columns = utils.get_column_names(dict(pipe.steps)["preprocessor"].transformers_[1][1].named_steps["encoder"], categorical_columns)
    ...
    def load_stage_file_from_local(session: snowflake.snowpark.Session, stage_name: str, 
        ...
        """
        utils.fetch_staged_file(session, stage_name, file_name, target_folder)
        if filetype == 'json':
    ...
    #     merged_config = utils.combine_config(notebook_config, predict_config)
    ...
    #     column_dict = utils.load_stage_file_from_local(session, stage_name, f"{output_profiles_ml_model}_{model_id}_column_names.json", '.', 'json')
    ...
    #     if len(timestamp_columns) == 0:
    #         timestamp_columns = utils.get_timestamp_columns(session, feature_table, index_timestamp)
    #     for col in timestamp_columns:
    ...
    #     pipe = utils.load_stage_file_from_local(session, stage_name, model_file_name, '.', 'joblib')
    #     onehot_encoder_columns = utils.get_column_names(dict(pipe.steps)["preprocessor"].transformers_[1][1].named_steps["encoder"], categorical_columns)
