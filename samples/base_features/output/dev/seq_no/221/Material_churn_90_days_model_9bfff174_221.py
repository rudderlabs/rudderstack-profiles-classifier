...
train_inputs = [
    """SELECT * FROM Material_rudder_user_base_features_0abcf2d7_221""",
]
...
if __name__ == "__main__":
    ...
    train_config = json.loads('{"data":{"eligible_users":null,"features_profiles_model":"rudder_user_base_features","inputs":["models/rudder_user_base_features"],"label_column":"is_churned_90_days","label_value":1,"output_profiles_ml_model":"churn_90_days_model","package_name":"feature_table","prediction_horizon_days":90},"preprocessing":{"ignore_features":["user_email","first_name","last_name"]}}')
    predict_config = json.loads('{"data":{"eligible_users":null,"features_profiles_model":"rudder_user_base_features","inputs":["models/rudder_user_base_features"],"label_column":"is_churned_90_days","label_value":1,"output_profiles_ml_model":"churn_90_days_model","package_name":"feature_table","prediction_horizon_days":90},"outputs":{"column_names":{"percentile":"percentile_churn_score_90_days","score":"churn_score_90_days"},"feature_meta_data":{"features":[{"description":"Percentile of churn score. Higher the percentile, higher the probability of churn","name":"percentile_churn_score_90_days"}]}},"preprocessing":{"ignore_features":["user_email","first_name","last_name"]}}')

    if should_train:
        T.train(creds, train_inputs, t_output_filename, train_config)
