...
USER_COUNT = 10000
PRODUCT_COUNT = 100
START_TIME = datetime.datetime(2024, 5, 1, 0, 0, 0) # The simulated journeys start from this time till DAYS_TO_SIMULATE days. 
...
sl_checkout = ['anonymous_id', 'tax', 'currency', 'channel', 'received_at', 'event_text', 'uuid_ts', 'country_code',
                 'user_id', 'locale',  'timestamp',  'shipping',
               'revenue',
               'event', 
               'products', 'payment_method', 
               'id', 'order_id',
               'original_timestamp', 'sent_at', 'checkout_id', 'cart_id',
               'context_page_url', 
               'context_session_id',
               'context_screen_height','context_screen_width',
               'context_screen_density','context_screen_inner_height','context_screen_inner_width', 'context_ip', 'context_session_start', 'total'
              ]
...
def generate_product_data():
    product_ids = random.sample(range(10*PRODUCT_COUNT,100*PRODUCT_COUNT), PRODUCT_COUNT)
    products = []
    for product_id in product_ids:
        #name = f"{fake.catch_phrase()} {fake.color_name()}"
        name = None
        cat = fake.random_element(elements=('Watches','Trousers',"Shoes",'Formal Shoes','Mens clothing','Shirts','Jeans'))
        path = f'/p/{product_id}'
        url = f"{webpage}{path}"
...
def generate_random_ts():
    ts = START_TIME + (datetime.timedelta(days=random.randint(0, DAYS_TO_SIMULATE)) + 
...
device_cols = ['CONTEXT_SCREEN_DENSITY', 'CONTEXT_SCREEN_WIDTH', 'CONTEXT_SCREEN_HEIGHT', 'CONTEXT_SCREEN_INNER_HEIGHT', 'CONTEXT_SCREEN_INNER_WIDTH']
device_specs=  [{'CONTEXT_SCREEN_DENSITY': 1, 
...
campaign_samples = [{'context_campaign_term': 'christmas sale',
...
urls = {"home": webpage, "cart": f"{webpage}/cart/view.html?ref_=nav_cart", "order_complete": f"{webpage}/order/success"}
...
def identify(user, event_time, campaign_id, device_id, ip, st_time, lib_version, url) -> dict:
    event = {}
    if not user.email:
...
def page(user: User, event_time: str, st_time: str, campaign_id: int, page_name: int, path: str, referrer_path: str, ip: str, product_id=None):
    event = {}
    event['id'] = generate_random_string(16)
    event['context_ip'] = ip
...
def order_complete_event(user, product_info, event_time, device_id, ip, st_time, page_url):
    event = {}
    user_data = asdict(user)
    event['context_ip'] = ip
    event["context_session_start"] = get_timestamp_string(st_time)
    ...
    event['payment_method'] = fake.random_element(elements=('paypal', 'amex', 'visa', 'mastercard'))
...
# Sample page call
samp_page_event = page(user=sample_user, event_time=st_time_, st_time = st_time_, campaign_id=0, page_name='home-page', path='/c/shoes', referrer_path = '/c/categories', ip=fake.ipv4(),  product_id = 4)
...
id_event = identify(sample_user, st_time_, 0, 2, ip_, st_time_, lib_versions[0], webpage)
...
checkout_event = order_complete_event(sample_user,p_,st_time_, 0, fake.ipv4(), st_time_, 'asdf')
...
paths = {"signup": signup_path, "login": login_path, "order_complete": order_complete_path, 'cart_add': cart_add_path}

def product_page_views(user, max_events, event_time, st_time, campaign_id, referrer_path, ip):
    events = []
    for _ in range(random.randint(1,max_events)):
        product_id = fake.random_int(min=0, max=(len(products)-1))
        product_event = page(user, event_time, st_time, campaign_id, 'product-view', None, referrer_path, ip, product_id)
        referrer_path = products[product_id]['path']
        events.append({"page": product_event})
        event_time = event_time + datetime.timedelta(minutes = random.randint(6, 60))  + datetime.timedelta(seconds = random.randint(10, 600))
    return events, event_time, referrer_path, product_id

def signin(user, event, event_time, session_start_time, campaign_id,device_id, referrer_path, ip):
    ...
    identify_event = identify(user, event_time, campaign_id, device_id, ip, session_start_time, lib_version, f"{webpage}{paths[event]}")
    page_event = page(user, event_time, session_start_time, campaign_id, event, paths[event], referrer_path, ip)
    signin_events = [{"page": page_event}, {"track": signin_track}, {"identify": identify_event}]
...
def cart_add(user, event_time, st_time, campaign_id, referrer_path, ip, product_id):
    ...
    cart_add_page_view = page(user, event_time, st_time, campaign_id, 'cart-add', cart_add_path, referrer_path, ip, product_id)
    cart_add_track = track(user, event_time, st_time, campaign_id, 'cart_add')
    referrer_path = cart_add_path
    return event_time, referrer_path, [{"page": cart_add_page_view}, {"track": cart_add_track}]

def order_completed_event(user, event_time, campaign_id, referrer_path, ip, device_id, st_time, product_id):
    event_time = event_time + datetime.timedelta(minutes=random.randint(10, 600)) + datetime.timedelta(seconds=random.randint(60, 600))
    order_completed_track = track(user, event_time, st_time, campaign_id, 'order_completed')
    order_completed_page = page(user, event_time, st_time, campaign_id, 'order_completed', order_complete_path, referrer_path, ip)
    product_info = {**products[product_id], "price": product_prices[product_id]}
    order_completed_event = order_complete_event(user, product_info, event_time, device_id, ip, st_time, f"{webpage}{referrer_path}")
    referrer_path = order_complete_path    
    return event_time, referrer_path, [{"page": order_completed_page}, {"track": order_completed_track}, {"order_completed": order_completed_event}]
...
def simulate_journey(user, journey_type):
    ...
    **Journeys:**
    ...
    if not user.last_seen_time:
        ...
        st_time = user.last_seen_time + datetime.timedelta(days = random.randint(14, 60)) + datetime.timedelta(minutes = random.randint(0, 600))
    ip = fake.ipv4()
    ...
    if journey_type <= 7:
        events = [{"page": page(user, st_time, st_time, campaign_id, 'home-page', None, None, ip)}]
        event_time = st_time + datetime.timedelta(days = random.randint(3, 21))
        referrer_path = None
        if journey_type <= 4:
            n_products_viewed_max = 5
            product_events, event_time, referrer_path, product_id = product_page_views(user, n_products_viewed_max, event_time, st_time,  campaign_id, referrer_path, ip)
            events.extend(product_events)
            if journey_type > 1:
                event_time, referrer_path, cart_add_events = cart_add(user, event_time, st_time, campaign_id, referrer_path, ip, product_id)
                events.extend(cart_add_events)
                if journey_type >2:
                    # signup if jounrey type is 3, login if journey type is 4
                    event_time, referrer_path, login_events = signin(user, 'login' if journey_type == 4 else 'signup', event_time,  st_time, campaign_id, device_id, referrer_path, ip)
                    events.extend(login_events)
                    # Add purchase event
                    event_time, referrer_path, order_complete_events = order_completed_event(user, event_time, campaign_id, referrer_path, ip, device_id, st_time, product_id)
                    events.extend(order_complete_events)
        else:
            # Add signup event followed by product page views
            event_time, referrer_path, signup_events = signin(user, 'signup', event_time, st_time, campaign_id, device_id, referrer_path, ip)
        ...
        events = []
        referrer_path=None
        event_time, referrer_path, login_events = signin(user, 'login', st_time, st_time, campaign_id, device_id, referrer_path, ip)
        product_events, event_time, referrer_path, product_id = product_page_views(user, 1, event_time, st_time, campaign_id, referrer_path, ip)
        event_time, referrer_path, cart_add_events = cart_add(user, event_time, st_time, campaign_id, referrer_path, ip, product_id)
        event_time, referrer_path, order_complete_events = order_completed_event(user, event_time, campaign_id, referrer_path, ip, device_id, st_time, product_id)
        events.extend([*login_events, *product_events, *cart_add_events, *order_complete_events])
    elif journey_type >= 9:
        # product page, product page, cart add
        referrer_path = None
        events = []
        product_events, event_time, referrer_path, product_id = product_page_views(user, 2, st_time, st_time, campaign_id, referrer_path, ip)
        event_time, referrer_path, cart_add_events = cart_add(user, event_time, st_time, campaign_id, referrer_path, ip, product_id)
        events.extend([*product_events, *cart_add_events])
        if journey_type == 10:
            # Add login, purchase
            event_time, referrer_path, login_events = signin(user, 'login', event_time, st_time, campaign_id, device_id, referrer_path, ip)
            event_time, referrer_path, order_complete_events = order_completed_event(user, event_time, campaign_id, referrer_path, ip, device_id, st_time, product_id)
...
journey_dist = {1: 0.35, 2: 0.13, 3: 0.04, 4: 0.04, 5: 0.05, 6: 0.13, 7: 0.04, 8: 0.04, 9: 0.14, 10: 0.04}
...
for i in range(USER_COUNT):
    ...
    for _ in range(n_journeys):
        if journey_id in (8, 10):
            journeys.append(simulate_journey(users[i], fake.random_element(elements=(5, 6))))
        journeys.append(simulate_journey(users[i], journey_id))
        if journey_id == 3:
            journeys.append(simulate_journey(users[i], fake.random_element(elements=(4, 8, 9))))
