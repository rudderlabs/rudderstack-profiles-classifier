...
class snowflakeProcessor(processor):
    ...
    #                     connector: Connector, 
    #                     session: Union[snowflake.snowpark.Session, redshift_connector.cursor.Cursor])-> None:
    #     super().__init__(trainer, connector)
    ...
    def prepare_feature_table(self, session,
        ...
        try:
            ...
            if self.trainer.eligible_users:
                feature_table = self.connector.get_table(session, feature_table_name, filter_condition=self.trainer.eligible_users)
            else:
                default_user_shortlisting = f"{self.trainer.label_column} != {self.trainer.label_value}"
                feature_table = self.connector.get_table(session, feature_table_name, filter_condition=default_user_shortlisting) #.withColumn(label_ts_col, F.dateadd("day", F.lit(prediction_horizon_days), F.col(index_timestamp)))
            arraytype_features = self.connector.get_arraytype_features(session, feature_table_name)
            ignore_features = utils.merge_lists_to_unique(self.trainer.prep.ignore_features, arraytype_features)
            high_cardinal_features = self.connector.get_high_cardinal_features(session, feature_table_name, self.trainer.label_column, self.trainer.entity_column, cardinal_feature_threshold)
            ...
            if len(timestamp_columns) == 0:
                timestamp_columns = self.connector.get_timestamp_columns(session, feature_table_name, self.trainer.index_timestamp)
            for col in timestamp_columns:
                feature_table = self.connector.add_days_diff(feature_table, col, col, self.trainer.index_timestamp)
            label_table = self.trainer.prepare_label_table(self.connector, session, label_table_name, label_ts_col)
    ...
    def prepare_and_train(self, material_names: List[Tuple[str]]):        
        ...
        for row in material_names:
            ...
            feature_table_instance, arraytype_features, timestamp_columns = self.prepare_feature_table(self.session,
                                                                    feature_table_name, 
                                                                    label_table_name,
        ...
        filtered_feature_table = self.connector.filter_feature_table(feature_table, self.trainer.entity_column, 
                                                                     self.trainer.index_timestamp, self.trainer.max_row_count)
