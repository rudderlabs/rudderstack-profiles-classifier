...
class CommonWarehouseConnector(Connector):
    ...
    def _get_default_eligible_users(
        self,
        ...
        best_filtered_table = feature_table

        booleantype_features: List[str] = input_column_types["booleantype"]
        total_positives = (feature_table[label_column] == label_value).sum()
        number_of_rows = len(feature_table)
        ...
        for bool_feature in booleantype_features:
            for bool_value in [True, False]:
                filtered_table = feature_table[
                    feature_table[bool_feature] == bool_value
                ]
                ...
                positives_after_filter = (
                    filtered_table[label_column] == label_value
                ).sum()
                recall = positives_after_filter / total_positives

                label_proportions = filtered_table[label_column].value_counts(
                    normalize=True
                )
                is_balanced = all(
                    (label_proportions >= min_proportion)
                    & (label_proportions <= max_proportion)
                )

                if is_balanced and recall > best_recall:
                    best_recall = recall
                    best_filtered_table = filtered_table
        ...
        return best_filtered_table

    def get_eligible_users(
        self, feature_table: pd.DataFrame, input_column_types: dict, **kwargs
    ):
        trainer: MLTrainer = kwargs.get("trainer", None)

        if trainer.eligible_users and trainer.eligible_users != "*":
            feature_table = feature_table.query(trainer.eligible_users)
        elif trainer.eligible_users == "*":
            pass
        else:
            feature_table = self._get_default_eligible_users(
                feature_table,
                input_column_types,
