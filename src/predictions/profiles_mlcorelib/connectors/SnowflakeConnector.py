...
class SnowflakeConnector(Connector):
    ...
    def _get_default_eligible_users(
        ...
        total_positives = feature_table.filter(
            F.col(label_column) == label_value
        ).count()
        number_of_rows = feature_table.count()
        ...
        for bool_feature in booleantype_features:
            for bool_value in [True, False]:
                filtered_table = feature_table.filter(F.col(bool_feature) == bool_value)
                if filtered_table.count() == 0:
                    logger.get().info(
                        f"No rows found for condition '{bool_feature} = {bool_value}' while filtering eligible users for training. Skipping..."
                    )
                    continue

                positives_after_filter = filtered_table.filter(
                    F.col(label_column) == label_value
                ).count()
                recall = positives_after_filter / total_positives

                total_count = filtered_table.count()
                label_proportions = (
                    filtered_table.groupBy(label_column)
                    .count()
                    .withColumn("proportion", F.col("count") / total_count)
                    .select("proportion")
                    .collect()
                )
                proportions = [row["PROPORTION"] for row in label_proportions]
                is_balanced = all(
                    p >= min_proportion and p <= max_proportion for p in proportions
                )
