    ...
    def _get_final_selector_sql(
        ...
        for conversion_name, value_flag in zip(conversion_name_list, value_flag_list):
            select_query = (
                ...
                + (
                    f""", coalesce(nullif(coalesce({conversion_name}_first_touch_conversion_value, 0), 0) / nullif(coalesce(cost, 0),0), 0) AS {conversion_name}_first_touch_roas,
                    coalesce(nullif(coalesce({conversion_name}_last_touch_conversion_value, 0), 0) / nullif(coalesce(cost, 0),0), 0) AS {conversion_name}_last_touch_roas """
                    if value_flag
                    else ""
                )
            )
