...
def train(creds: dict, inputs: str, output_filename: str, config: dict) -> None:
    ...
    figure_names = {"roc-auc-curve": f"01-test-roc-auc-{output_profiles_ml_model}.png",
                    "pr-auc-curve": f"02-test-pr-auc-{output_profiles_ml_model}.png",
                    "lift-chart": f"03-test-lift-chart-{output_profiles_ml_model}.png",
                    "feature-importance-chart": f"04-feature-importance-chart-{output_profiles_ml_model}.png"}
    ...
    def train_sp(session: snowflake.snowpark.Session,
        ...
        train_x, train_y, test_x, test_y, val_x, val_y = utils.split_train_test(session, feature_table, label_column, entity_column, output_profiles_ml_model, train_size, val_size, test_size)
        ...
        result_dict = {"model_id": model_id,
                        "model_name_prefix": output_profiles_ml_model,
                        "prob_th": prob_th,
        ...
        model_file = os.path.join('/tmp', f"{output_profiles_ml_model}_{model_file_name}")
        ...
        column_name_file = os.path.join('/tmp', f"{output_profiles_ml_model}_{model_id}_column_names.json")
    ...
    model_hash, creation_ts = utils.get_latest_material_hash(session, material_table, features_profiles_model)

    if start_date == None or end_date == None:
        start_date, end_date = utils.get_date_range(creation_ts, prediction_horizon_days)

    material_names, training_dates = utils.get_material_names(session, material_table, start_date, end_date, package_name, features_profiles_model, model_hash, material_table_prefix, prediction_horizon_days, output_filename)
    ...
    feature_table_name_remote = f"{output_profiles_ml_model}_features"
