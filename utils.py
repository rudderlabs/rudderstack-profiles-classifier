...
def split_train_test(feature_df: pd.DataFrame, 
    ...
    Args:
        feature_table (pd.DataFrame): feature table from the retrieved material_names tuple
...
def plot_roc_auc_curve(session, pipe, stage_name, test_x, test_y, chart_name, label_column, target_path)-> None:
    """
    ...
    if not isinstance(session, str):
        ...
        figure_file = os.path.join(target_path, f"{chart_name}")
        plt.savefig(figure_file)
    plt.clf()

def plot_pr_auc_curve(session, pipe, stage_name, test_x, test_y, chart_name, label_column, target_path)-> None:
    """
...
def plot_lift_chart(session, pipe, stage_name, test_x, test_y, chart_name, label_column, target_path)-> None:
    """
...
def plot_top_k_feature_importance(session, pipe, stage_name, target_path, train_x, numeric_columns, categorical_columns, chart_name, top_k_features=5)-> None:
    """
    ...
    try:
        ...
        if not isinstance(session, str):
            ...
            shap_importance.to_csv(f"data/FEATURE_IMPORTANCE.csv", index=False)
